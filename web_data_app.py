...
def hello_there(name):
    ...
    if match_object:
        clean_name = match_object.group(0).capitalize()
    else:
        clean_name = "Friend"
...
def book_data():
    ...
    book_data['Prices'] = book_data['Prices'].apply(lambda x: f'${x:,.2f}')
    book_data['Sale Prices'] = book_data['Sale Prices'].apply(lambda x: f'${x:,.2f}')
    ...
    return render_template('template.html',  tables=[book_data.to_html(classes='data', index=False)], 
                           titles=book_data.columns.values
                           )
...
def learn():
    ...
    return """One of my favourite things learned in ENSF 692 was list comprehensions. Coming from a background in strongly typed languages, it is really enjoyable to have one line of code do so much!"""
